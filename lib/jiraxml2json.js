...
module.exports.toJson = function(jiraXml) {
    ...
    for(var i = 0; i < otherTasks.length; i++) {
        ...
        otherTasksTimeSpentHour += otherTask.timespent.hours;
    }
    parentOfOtherTask.timespent = new Object();
    parentOfOtherTask.timespent.hours = otherTasksTimeSpentHour;
    ...
    for(var i = 0; i < parentTasks.length; i++) {
        ...
        totalTimeSpentHour += parentTask.timespent.hours;
    }
    result.timespent.hours = totalTimeSpentHour;
...
function assignItemAttributeToTask(task, item, taskType) {
    ...
    task.key = new Object();
    task.key.id = item.key.id;
    task.key.name = item.key.$t;
    task.timespent = new Object();
    if(taskType == 'parent') { // Parent.
        if (item.aggregatetimespent && item.aggregatetimespent.seconds) {
            task.timespent.hours = calculateTimespentHour(item.aggregatetimespent.seconds);
        } else {
            task.timespent.hours = 0.0;
        }            
    } else { // Child.
        if (item.timespent && item.timespent.seconds) {
            task.timespent.hours = calculateTimespentHour(item.timespent.seconds);
        } else {
            task.timespent.hours = 0.0;
        }
    }
...
/**
 * Calculate timespent seconds to hours
